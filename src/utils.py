...
def requests_params_hh(name_vacancie: str, one_page_count: int = 5, all_page_count: int = 1):
    params = {
        "text": name_vacancie,
...
def get_vacancies_hh(params: dict):
    ...
    if response.status_code == 200:
        ...
        for vacancy in vacancies:
            return (print(vacancy["name"], vacancy["url"], vacancy["area"]["name"], vacancy["snippet"]))
...
def requests_params_sj(name_vacancie: str):
...
def get_vacancies_sj(api_key: str, params: dict):
    ...
    if response.status_code == 200:
        ...
        for vacancy in vacancies:
            #print(vacancy)
            print(vacancy["profession"],vacancy["profession"], vacancy["link"], vacancy["payment_from"], vacancy["payment_to"], vacancy["town"]["title"])
    else:
        print("Error:", response.status_code)


get_vacancies_sj(superjob_api_key, requests_params_sj("python"))
get_vacancies_hh(requests_params_hh("python"))
